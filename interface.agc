...
function DrawAllButtons()
	...
	for index = 0 to (NumberOfButtonsOnScreen-1)
		if (  GetSpriteExists( ButtonSprite[ ButtonIndex[index] ] ) = 1  )
			...
			for posX = -2 to 2 step 1
				for posY = -2 to 2 step 1
					if ButtonScale[index] = 1
						SetTextSize(textStartIndex+outlineIndex, 34+5+7)
					else
						SetTextSize(textStartIndex+outlineIndex, 30+5+7)
					endif
					...
					SetTextColor(textStartIndex+outlineIndex, 230, 230, 230, 255)
			...
			if ButtonScale[index] = 1
				SetTextSize(textStartIndex, 35+5+7)
			else
				SetTextSize(textStartIndex, 31+5+7)
			endif

			SetTextColorAlpha(textStartIndex, 255)

			SetTextColor(textStartIndex, 0, 0, 0, 255)
